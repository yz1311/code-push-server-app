...
export default class YZScrollableTabItem extends Component<IProps,IState> {
  static propTypes = {
    ...
    underlineStyle:PropTypes.any //底部指示器的样式
  }
  ...
  render() {
    ...
    return (
      <View style={[{flexDirection:'row',backgroundColor:gColors.themeColor},this.props.containerStyle]}>
        {this.props.tabs.map((tab, index) => {
          const isTabActive = this.props.activeTab === index;
          const renderTab = this.props.renderTab || this.renderTab;
          return this.renderTab(this.props.tabDatas[index],index,isTabActive,this.props.goToPage);
        })}
  ...
  {
    const { activeTextColor, inactiveTextColor, textStyle,tabDatas} = this.props;
    const textColor = isTabActive ? activeTextColor : inactiveTextColor;
    const fontWeight = isTabActive ? 'bold' : 'normal';
    return(
      <TouchableOpacity key={index} activeOpacity={activeOpacity} onPress={()=>this.props.goToPage(index)} style={{flex:1}}>
        <View style={{flexDirection:'row',justifyContent:'center',alignItems:'center',paddingVertical:20}}>
          <Text style={[{color:textColor,fontSize:gFont.size16},this.props.textStyle]}>{tabDatas[index].Name}</Text>
        </View>
      </TouchableOpacity>
    );
  }
